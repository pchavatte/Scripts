...
#>
$domain = 'emea.adecco.net'
$server = 'DCINTNL000532.emea.adecco.net'
Try {
    Import-Module GroupPolicy -ErrorAction Stop
    $gpos = @(Get-GPO -All -Domain $domain -Server $server )   #| Where-Object {$_.DisplayName -like 'FR*' }
    $count = $gpos.count
    $i=0
    ForEach ($gpo in $gpos) {
        ...
        $xml = [xml]($gpo | Get-GPOReport -Domain $domain -Server $server -ReportType XML)
        #User logon script
...
#Get-GPO -All | Where-Object { $_ | Get-GPOReport -Domain $domain -Server $server -ReportType HTML | Select-String "<LinksTo>" }